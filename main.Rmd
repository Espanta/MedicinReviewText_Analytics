...
```{r}
dim(trial[nchar(trial$abstract)==0,])
...
2) Convert corpusTitle and corpusAbstract to lowercase.Convert them to plaintext document.
...
```{r}
titleWords <- as.data.frame(as.matrix(sparseTitle))
dim(titleWords)
abstractWords <- as.data.frame(as.matrix(sparseAbstract))
dim(abstractWords)
```

How many terms remain in titleWords after removing sparse terms (aka how many columns does it have)?

How many terms remain in abstractWords?
...
```{r}
which.max(colSums(abstractWords))
```
...
colnames(titleWords) <- paste0("T", colnames(titleWords))
```
...
```{r}
dtm <- cbind(abstractWords, titleWords)
```
...
set.seed(144)
lubripack("caTools")
index <- sample.split(dtm$trial, 0.7)
train <- dtm[index,]
test <- dtm[!index,]
...
```{r}
pred1 <-predict(CART1, newdata = train)
max(pred1[,2])
```
...
**Answer:** The maximum predicted probability will likely be exactly the same in the testing set.  
...
```{r}
predTest <-predict(CART1, newdata = test)
...
ROCPerf <- performance(ROCRPred, "auc")
